...
import { Plus, Copy, Check, Edit3 } from 'lucide-react';
...
export const CategoryCard: React.FC<CategoryCardProps> = ({
  ...
  return (
    <div
      ...
      className={`flex flex-col rounded-xl ${theme.cardBorder} bg-white transition-all duration-150 overflow-hidden`}
    >
      ...
      <div className={`px-3.5 py-2.5 ${theme.headerBg} ${theme.headerBorder} flex items-center justify-between gap-2`}>
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            <h3 className="text-xs sm:text-sm font-black tracking-wider uppercase font-industrial truncate text-slate-900">
              {category.name}
            </h3>
            {category.meaning && (
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          ...
          {items.length > 0 && (
            <button
              ...
              className="p-1 rounded hover:bg-black/5 text-slate-500 hover:text-slate-900 transition-colors no-print cursor-pointer"
              title="Copy all KSK numbers in this category"
            >
      ...
      {/* Card Content */}
      <div className="p-3 flex-1 flex flex-col justify-start min-h-[90px]">
        {items.length === 0 ? (
          <div
            onClick={() => onQuickAdd(category.id)}
            className="flex-1 flex flex-col items-center justify-center py-4 px-2 border border-dashed border-slate-200 rounded-lg text-slate-400 hover:border-slate-300 hover:text-slate-600 cursor-pointer transition-colors bg-slate-50/50"
          >
            <span className="text-xs font-medium italic">Empty</span>
            <span className="text-[11px] text-blue-600 font-bold mt-0.5 no-print">+ Add KSK</span>
          </div>
            ...
            {items.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`w-full text-left px-3 py-1.5 rounded-lg border font-mono text-xs sm:text-sm font-bold tracking-wider transition-all duration-150 flex items-center justify-between group cursor-pointer ${
                    matched
                ...
                >
                  ...
                  <Edit3 className="w-3 h-3 text-slate-400 group-hover:text-blue-600 transition-colors shrink-0 ml-1 no-print" />
                </button>
              );
            })}
          </div>
        ) : (
          /* One Line Mode (KSKs separated by -) */
          <div className="flex flex-wrap items-center gap-1.5 p-2 bg-slate-50 rounded-lg border border-slate-200 text-xs sm:text-sm font-mono tracking-wide">
            {items.map((item, idx) => {
              const matched = isMatched(item.kskNumber);
              return (
